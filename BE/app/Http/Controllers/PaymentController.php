...
class PaymentController extends Controller
{
    ...
    public function storeOrder(StoreOrderRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => ((float) ($order->amount ?? 0) === 0.0 && ($order->payment_method ?? null) === 'coupon_trial')
                ? 'Ghi danh học thử miễn phí thành công.'
    ...
    public function vnpayReturn(Request $request)
    {
        ...
        try {
            $this->paymentService->vnpayReturn($request->all());
