...
class WithdrawalRequestedAdminMail extends Mailable
{
    use Queueable;
    ...
    public function build(): self
    {
        ...
        $phone = e($this->instructor->phone ?? 'Chưa cập nhật');
        ...
        $requestedAt = $this->withdrawal->requested_at 
            ? $this->withdrawal->requested_at->format('d/m/Y H:i:s') 
            : date('d/m/Y H:i:s');

        $frontendUrl = rtrim(config('app.frontend_url', env('FRONTEND_URL', 'http://localhost:5173')), '/');
        $adminReviewUrl = "{$frontendUrl}/admin/withdrawals";
        ...
        return $this->subject("[MindHub Admin] Yêu cầu rút tiền mới #{$this->withdrawal->id} từ: {$instructorName}")
            ->html("
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                ...
                .badge { display: inline-block; background-color: #eff6ff; color: #2563eb; font-weight: 700; font-size: 13px; padding: 6px 14px; border-radius: 20px; margin-top: 4px; }
                ...
                .amount-box { text-align: center; background: #3b82f6; color: #ffffff; padding: 16px; border-radius: 12px; font-weight: 800; font-size: 22px; margin-bottom: 16px; }
                ...
                .btn { display: inline-block; background-color: #2563eb; color: #ffffff !important; font-weight: 700; text-decoration: none; padding: 14px 32px; border-radius: 12px; font-size: 15px; box-shadow: 0 4px 10px rgba(37,99,235,0.25); }
        ...
        <body>
            <div class='container'>
                <div class='header'>
                    ...
                    <div class='badge'>💸 Yêu cầu rút tiền mới</div>
                </div>
                <div class='content'>
                    <h2>Kính gửi Quản trị viên,</h2>
                    <p>Giảng viên <strong>{$instructorName}</strong> vừa nộp yêu cầu chuyển tiền/rút tiền trên nền tảng <strong>MindHub</strong>.</p>
                    
                    <div class='card'>
                        <div class='amount-box'>{$amountFormatted}</div>
                        <div class='title'>Thông tin yêu cầu rút tiền #{$this->withdrawal->id}</div>
                        <div class='info-item'>👤 Giảng viên: <strong>{$instructorName}</strong></div>
                        <div class='info-item'>✉️ Email: <strong>{$instructorEmail}</strong></div>
                        <div class='info-item'>📞 Số điện thoại: <strong>{$phone}</strong></div>
                        <div class='info-item'>📌 Loại yêu cầu: <strong>{$typeLabel}</strong></div>
                        <div class='info-item'>🏦 Ngân hàng: <strong>{$bankName}</strong></div>
                        ...
                        <div class='info-item'>⏰ Thời gian gửi: <strong>{$requestedAt}</strong></div>
                    </div>

                    <p>Vui lòng đăng nhập hệ thống Quản trị để kiểm tra và xử lý duyệt yêu cầu này:</p>
                    
                    <div class='btn-wrapper'>
                        <a href='{$adminReviewUrl}' class='btn' target='_blank'>Xem & Phê Duyệt Yêu Cầu Rút Tiền</a>
                    </div>
                </div>
                <div class='footer'>
