...
final class AdminRevenueRepository
{
    ...
    {
        return $this->query($filters)->select('sale_channel', DB::raw('SUM(gross_amount) gross_amount'), DB::raw('SUM(instructor_amount) instructor_amount'), DB::raw('SUM(platform_fee_amount) platform_fee_amount'), DB::raw('COUNT(*) total'))->groupBy('sale_channel')->get()->toArray();
    }
