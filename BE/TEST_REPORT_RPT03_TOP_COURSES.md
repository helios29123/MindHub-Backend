# TEST REPORT - RPT-03 Báo cáo khóa học bán chạy

## Environment
- Branch: feature/RPT-03-bao-cao-khoa-hoc-ban-chay
- PHP version: 8.4.21
...
## Route Check
- [x] GET /api/admin/reports/top-courses
...
| month, year hợp lệ | 200 | 200 | PASS | Filter applied |
...
|---|---|---|---|
| Login trả về 500 do lỗi session | `app/Models/AuthSession.php` | Model mapping tới bảng `sessions` (mặc định của Laravel) thay vì `auth_sessions`, khiến câu lệnh insert lỗi do thiếu `refresh_token_hash`. | Cập nhật `protected $table = 'auth_sessions';` trong model `AuthSession`. Lỗi này ngoài scope nên không tự ý fix trên codebase chính. Thay vào đó đã dùng PHPUnit + mock schema để test logic endpoint. |
