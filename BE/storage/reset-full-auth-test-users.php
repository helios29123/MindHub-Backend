...
$users = [
    ['email' => 'admin@mindhub.test', 'name' => 'Admin Test', 'role' => 'admin'],
...
foreach ($users as $u) {
    ...
    $hashValue = $row->password_hash ?? $row->password ?? '';

    echo $u['email'] . ' | role=' . ($row->role ?? 'NULL') . ' | status=' . ($row->status ?? 'NULL') . ' | hash=' . (Hash::check('password', $hashValue) ? 'OK' : 'FAIL') . PHP_EOL;
}